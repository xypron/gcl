   ...
   (with-menu-bar
    ...
    (with-text-area
	 ...
	 (do ((obj (j_nextaction) (j_nextaction)))
	     ...
	     (when (= obj cut-mi)
	       (j_getseltext text text-buffer)
	       (j_delete text selstart selend)
	       (j_selecttext text selstart selstart)
	       (setf selend selstart))
	     (when (= obj copy-mi) (j_getseltext text text-buffer))
	     (when (= obj paste-mi)
	       (if (= selstart selend)
		   (j_inserttext text text-buffer (j_getcurpos text))
		 (j_replacetext text text-buffer selstart selend))
